...
// 首頁「去過的地方」足跡地圖（2026-08-10 新增，同日傍晚從 /blog 搬到首頁）
// 標籤裡是地點的那些，在首頁地圖上變成一塊可以點亮的國家色塊。
// 哪個標籤對應哪個國家，用內文的表格維護（Obsidian 的表格可以像 Excel
// 一樣按 Tab 換格編輯）。沒寫進表格的標籤不會不見，它們照常出現在
// /blog 的標籤列——那一列是全部標籤，跟這張地圖各做各的事。
const tagMap = defineCollection({
	loader: glob({ base: "./src/content/編輯", pattern: "標籤地圖.md" }),
	schema: z.object({
		heading: z.string(),
	}),
	// 內文：說明段落 ＋ 一個「標籤／國家／緯度／經度」的表格
});
